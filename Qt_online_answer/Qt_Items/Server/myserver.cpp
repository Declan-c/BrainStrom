...
#endif
    _db = new DB(NULL, "root", "20010221", "brainstrom");
    initRankMap();
}
// 客户端连接事件
...
void Myserver::closeEvent(TcpSocket *s, short)
{
    ...
    for (; it != _users.end(); it++)
    {
        ...
        {
            _users.erase(it);
            spdlog::get("log")->info("用户{} [{}:{}] 退出", it->second->getName(), s->getIp(), s->getPort());

            // 处理用户在等待匹配时意外退出，将用户行排位列表中移除
            // {
    ...
    spdlog::get("log")->info("客户端[{}:{}] 退出", s->getIp(), s->getPort());
}
...
void Myserver::Register(TcpSocket *s, const Json::Value &inJson)
{
    ...
    // 检测用户是否存在
    char sql[1024] = {0};
    sprintf(sql, "select * from user1 where name = '%s' and passwd = '%s'", userName.c_str(), passwd.c_str());
    ...
    else
    {
        ...
        sprintf(sql, "insert into user1(name,passwd,m_rank) values('%s','%s',0)", userName.c_str(), passwd.c_str());
        bool ret = _db->db_select(sql, outJson);
        if (!ret)
        {
            result = ERROR;
...
void Myserver::Login(TcpSocket *s, const Json::Value &inJson)
{
    ...
    if (outJson.isMember("name")) // 用户存在，表明已经注册过了
    {
        ...
        else
        {
            ...
            spdlog::get("log")->info("用户{} [{}:{}] 登录", userName, s->getIp(), s->getPort());
        }
    }
    else
    {
...
void Myserver::GetQuestion(TcpSocket *s, int flag)
{
    Json::Value json;
    char sql[1024] = {0};
    memset(sql, 0, sizeof(sql));

    if (GETMATHQUESTION == flag)
    {
    ...
    {
        sprintf(sql, "select * from chinese_question1 order by rand() limit %d", QUESTION_NUM);
    ...
    {
        sprintf(sql, "select * from english_question1 order by rand() limit %d", QUESTION_NUM);
...
void Myserver::startRank(TcpSocket *first, TcpSocket *second)
{
    ...
    spdlog::get("log")->info("({})和({})的对决题目：{}\n", first->getUserName(), second->getUserName(), json.toStyledString());
}
...
void Myserver::RankResult(TcpSocket *s, const Json::Value &inJson)
{
    ...
    sprintf(sql, "update user1 set m_rank = %d where name = '%s'", user->getRank(), user->getName());
    _db->db_exec(sql);
...
{
    std::unique_lock<std::mutex> lock(_rankLock);
    for (auto it = _rankQueue.begin(); it != _rankQueue.end(); ++it)
    {
        if (s == it->second)
        {
            _rankQueue.erase(it);
            spdlog::get("log")->info("{}取消了匹配", s->getUserName());
            break;
        }
    }
}